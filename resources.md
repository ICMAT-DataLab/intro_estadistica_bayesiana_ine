---
title: Recursos
nav_order: 3
---

# 📚 Recursos y Materiales
{: .no_toc }

## Tabla de Contenidos
{: .no_toc .text-delta }

1. TOC
{:toc}

---

## Materiales del Curso

Los materiales de cada sesión (diapositivas, ejercicios y código) estarán disponibles aquí a medida que avance el curso.

| Sesión | Tema | Materiales |
|--------|------|------------|
| 1 | Introducción, proporciones | [Slides T1](/slides/BayesINE1.pdf){: .btn .btn-sm} [Slides T2](/slides/BayesINE2.pdf){: .btn .btn-sm} <br> [Lab 1](/code/BayesINE1.zip){: .btn .btn-sm} [Lab 2](/code/BayesINElab2.zip){: .btn .btn-sm} |
| 2 | Medias, modelos de regresión lineal y jerárquicos | [Slides T3](/slides/BayesINE3.pdf){: .btn .btn-sm} [Slides T4](/slides/BayesINE4.pdf){: .btn .btn-sm} <br> [Lab 3](/code/BayesINE3.zip){: .btn .btn-sm} [Lab 4](/code/BayesINElab4.zip){: .btn .btn-sm}  |
| 3 | Series temporales, modelos lineales generalizados y regresión logística | Slides: Próximamente <br> Código: Próximamente |
| 4 | Métodos computacionales | Slides: Próximamente <br> Código: Próximamente |
| 5 | Análisis de decisiones y aplicaciones prácticas | Slides: Próximamente <br> Código: Próximamente |

---

## Referencias Bibliográficas

### 📖 Bibliografía Básica

- **Gelman, Carlin, Stern, Dunson, Vehtari (2014)** - Bayesian Data Analysis, CRC.
- **French, Ríos Insua (2000)** - Statistical Decision Theory, Wiley.
- **Hoff (2009)** - A First Course in Bayesian Statistical Methods, Springer.
- **Gelman, Hill, Vehtari (2021)** - Regression and other stories, Cambridge UP.
- **Ríos Insua, Ruggeri, Wiper (2012)** - Bayesian Analysis of Stochastic Processes, Wiley.
- **Albert (2009)** - Bayesian Computation with R. Springer.
- **Marin, Robert (2013)** - Bayesian Essentials with R. Springer.
- **McElreath (2019)** - Statistical Rethinking. CRC.
- **Ríos Insua et al. (2000)** - BayOff. Bayesian methods in Official Statistics. EUROSTAT.
- **Agresti, Kateri, Grove, Mira (2026)** - Bayesian Statistics: A primer for data scientists with Python and R.

### 🛠️ Herramientas y Software

- [Stan](https://mc-stan.org/) - Plataforma de modelado probabilístico de alto rendimiento.
- **R** - Entorno de computación estadística y librerías asociadas.
- Slides y Notebooks en R - (Se facilitarán durante el curso).

### 🔗 Otros Cursos del DataLab

- [Intro to Machine Learning](https://datalab-icmat.github.io/courses_stats.html#Introduction_to_Machine_Learning)
- [Bayesian Data Science](https://datalab-icmat.github.io/courses_stats.html#Bayesian_Data_Science)

{: .note }
> ¿Tienes algún recurso que quieras compartir? Puedes abrir un issue en nuestro [repositorio de GitHub](https://github.com/ICMAT-DataLab/intro_estadistica_bayesiana_ine).
